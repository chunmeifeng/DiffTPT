...
class diffuData(Dataset):
    def __init__(self, data_root, img_list, label_list, trainsform, augmode='difftpt', diffu_ratio=0.5, view_num=7):#view_num=63
...
def get_data_loader(data_transform, args=None):
    ...
    with open(os.path.join(args.diff_root, 'selected_data_list.txt'), 'r') as f:
        for line in f.readlines():
            ...
            label_list.append(imagenet_r_fold.index(read_img.split('/')[-2])) #imagenet-r
            #label_list.append(ucf101_classes.index(read_img.split('/')[-2]))
...
def test_time_tuning_difftpt(model, inputs, optimizer, scaler, args, cache_keys=None, cache_values=None):

    if args.cocoop:
        image_feature, pgen_ctx = inputs
        pgen_ctx.requires_grad = True
        optimizer = torch.optim.AdamW([pgen_ctx], 1e-4)
    
    if cache_keys is not None or hasattr(model, 'adapter'):
        ...
        with torch.cuda.amp.autocast():
            if args.cocoop:
                ...
                if cache_keys is not None:
                    output, _ = model(inputs, cache_keys, cache_values) 
                else:
                    output = model(inputs)
